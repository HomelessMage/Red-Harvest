	...
	if (_CHgroupSize==5) then {_CHgroupArray=[16,20]};
{
	...
	if (isnil "_eosMarkers") then {_eosMarkers=[];};
		...
		null = [_x,[_PApatrols,_PAgroupArray],[_LVehGroups,_LVgroupArray],[_AVehGroups],[_CHGroups,_CHgroupArray],_settings,_basSettings] execVM "modules\eos\core\b_core.sqf";
}foreach _JIPmkr;
