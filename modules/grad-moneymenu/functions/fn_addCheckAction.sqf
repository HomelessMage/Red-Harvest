...
_actionDesc = if (_target in grad_moneymenu_atmObjects) then {localize "STR_TAG_checkBalance"} else {localize "STR_TAG_checkCash"};
...
_code = if (_target in grad_moneymenu_atmObjects) then {
    {
        _hint = [
            format [localize "STR_TAG_bankBalance" + " <t color='#00FF00'>%1</t> Р", player getVariable ["grad_moneymenu_myBankBalance",0]],
