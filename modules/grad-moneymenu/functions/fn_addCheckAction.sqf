...
_actionDesc = if (_target in grad_moneymenu_atmObjects) then {"Проверить баланс"} else {"Проверить наличные"};
...
_code = if (_target in grad_moneymenu_atmObjects) then {
    {
        _hint = [
            format ["Банковский баланс" + " <t color='#00FF00'>%1</t> Р", player getVariable ["grad_moneymenu_myBankBalance",0]],
