...
_saveProgress = ["SaveProgress","Сохранить прогресс","\a3\Ui_f\data\GUI\Rsc\RscDisplayArcadeMap\icon_saveas_ca.paa",{[true, 30, gradTriggerGUER] call grad_persistence_fnc_saveMission},{side player == guer},{},[parameters],[0,0,0], 100] call ace_interact_menu_fnc_createAction;  
[this, 0, ["ACE_MainActions"], _saveProgress] call ace_interact_menu_fnc_addActionToObject; 
 
_healPlayer = ["HealPlayer","Получить медицинскую помощь","\a3\ui_f\data\igui\cfg\simpletasks\types\Heal_ca.paa",{[player] call ace_medical_treatment_fnc_fullHealLocal},{side player == guer},{},[parameters],[0,0,0], 100] call ace_interact_menu_fnc_createAction;  
...
_deployToMHQ = ["MHQDeploy","Высадка на КШМ","\a3\ui_f\data\igui\cfg\simpletasks\types\car_ca.paa",{
	player moveInCargo teleportItemGUER
	},{side player == independent},{},[parameters],[0,0,0], 100] call ace_interact_menu_fnc_createAction;
...
_giveRecruitEquipment = ["RecruitEquipment","Получить базовую экипировку","\a3\ui_f\data\igui\cfg\simpletasks\types\rearm_ca.paa",{
	...
	_backpackArray = ["UK3CB_B_Hiker_Camo", "UK3CB_B_Hiker"];
...
_openTrashBin = ["OpenStorage","Мусорный бак","\a3\ui_f\data\igui\cfg\simpletasks\types\box_ca.paa",{ 
 trashBinGUER setpos position (_this select 1); 
...
_removeTrashFromBin = ["RestoreEquipment","Уничтожить мусор в баке","\A3\ui_f\data\map\groupicons\waypoint.paa",{ 
 clearMagazineCargoGlobal trashBinGUER; 
 clearWeaponCargoGlobal trashBinGUER; 
 clearItemCargoGlobal trashBinGUER; 
...
_removeTrash = ["RemoveTrash","Уничтожить мусор на базе","\A3\ui_f\data\map\groupicons\waypoint.paa",{ 
  _radius = 1000; 
  _trashArray = gradTriggerGUER nearObjects ["GroundWeaponHolder", _radius]; 
  for "_i" from 0 to count _trashArray - 1 do { 
    deleteVehicle (_trashArray select _i); 
  }; 
  hintSilent "Мусор на базе удалён!"; 
},{side player == independent}] call ace_interact_menu_fnc_createAction; 
...
[this, 0, ["ACE_MainActions"], _openArsenal] call ace_interact_menu_fnc_addActionToObject; 
 ...
 lootBoxGUER setpos position (_this select 1); 
 (_this select 1) action ["Gear", lootBoxGUER] 
 },{ 
  side player == independent 
  },{},[parameters],[0,0,0], 100] call ace_interact_menu_fnc_createAction; 
