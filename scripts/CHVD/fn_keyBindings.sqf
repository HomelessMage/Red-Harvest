...
// systemChat format ["Нажмите <%1%2> чтобы открыть меню настройки дальности видимости.", _SCAstring, [[_openSettingsKey select 0] call BIS_fnc_keyCode] call CBA_fnc_capitalize];

systemChat format [localize "STR_TAG_press" + " <%1%2> " + localize "STR_TAG_toOpenDistanceDrawSetting", _SCAstring, [[_openSettingsKey select 0] call BIS_fnc_keyCode] call CBA_fnc_capitalize];